...
void accept_request(void *arg)
{
    ...
    char *query_string = NULL;
	 ...
	 sprintf(path, "htdocs%s", url);				// 在tinyhttpd 中服务器文件是在htdocs 文件夹下
	 ...
	 if (stat(path, &st) == -1) {
		  ...
		  while ((numchars > 0) && strcmp("\n", buf))  /* read & discard headers  当buf 数组中只包含 "\n" 是，通过strcmp 函数返回为0，结束循环 */
			numchars = get_line(client, buf, sizeof(buf));
...
int startup(u_short *port)
{
	 ...
	 if ((setsockopt(httpd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) < 0)  			//  允许套接口和一个已在使用中的地址捆绑
     {  
